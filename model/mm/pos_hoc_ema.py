...
def maybe_coerce_dtype(t, dtype):
    ...
    return t.to(dtype)
def inplace_copy(tgt: Tensor, src: Tensor, *, auto_move_device = False, coerce_dtype = True):
    if auto_move_device:
...
def inplace_lerp(tgt: Tensor, src: Tensor, weight, *, auto_move_device = False, coerce_dtype = True):
    if auto_move_device:
        src = src.to(tgt.device)

    if not isinstance(weight, torch.Tensor):
        weight = torch.tensor(weight, dtype=tgt.dtype, device=tgt.device)
    else:
        weight = weight.to(dtype=tgt.dtype, device=tgt.device)
...
class PostHocEMA(Module):
    ...
    def synthesize_ema_model(
        ...
        for ind, (checkpoint, weight) in tqdm(enumerate(zip(checkpoints, weights.tolist()))):
            is_first = ind == 0
