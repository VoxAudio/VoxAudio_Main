...
from model.mm.ext.autoencoder.edm2_utils import MPConv1D
from model.mm.ext.autoencoder.vae_modules import (AttnBlock1D, Downsample1D, ResnetBlock1D,
...
class VAE(nn.Module):
    ...
    def __init__(
        ...
        *,
        ...
        super().__init__()
        if data_dim == 80:
            mean = torch.tensor(DATA_MEAN_80D, dtype=torch.float32)
            std = torch.tensor(DATA_STD_80D, dtype=torch.float32)
        elif data_dim == 128:
            mean = torch.tensor(DATA_MEAN_128D, dtype=torch.float32)
            std = torch.tensor(DATA_STD_128D, dtype=torch.float32)
        else:
            raise ValueError(f"Unsupported data_dim: {data_dim}")

        self.register_buffer('data_mean', mean.view(1, -1, 1))
        self.register_buffer('data_std', std.view(1, -1, 1))
