...
from megatron.model.audiovae.base_model import BaseModel
from megatron.model.audiovae.codec_mixin import CodecMixin
from torch import Tensor, nn
from torch.nn import functional as F
from torch.nn.utils.parametrizations import weight_norm
from torch.nn.utils.parametrize import remove_parametrizations
from torch.nn import Parameter 
from torch import Tensor, exp, sin, pow
            ...
            if n_t_layer == 0
                ...
                WindowLimitedTransformer(
                    ...
                    config=transformer_general_config(
                        ...
                        n_head=dim_out // 64,
                        dim=dim_out,
                        intermediate_size=dim_out * 3,
                    ),
                )
            )
        )
        self.block = nn.Sequential(
            ResidualUnit(dim_in, dilation=1, causal=causal),
...
class Encoder(nn.Module):
    def __init__(
        ...
        self.block = [conv_class(1, 24, kernel_size=7, padding=3)]

        # Create EncoderBlocks that double channels as they downsample by `stride`
        for n_layer, (stride, n_t_layer) in enumerate(zip(strides, n_transformer_layers)):
            d_model *= 2
            if n_layer == 0:
                self.block += [
                    EncoderBlock(
...
class DAC(BaseModel, CodecMixin):
    def __init__(
        ...
        self.encoder_hop = np.prod(encoder_rates)
        ...
        self.encoder_frame = self.encoder_hop * 4
        self.decoder_frame = self.decoder_hop * 4
    ...
    def encode(
        ...
        **kwargs,
        ...
        length = audio_data.shape[-1]
        right_pad = math.ceil(length / self.encoder_frame) * self.encoder_frame - length
        audio_data = nn.functional.pad(audio_data, (0, right_pad))
        if audio_lengths is None:
            audio_lengths = torch.LongTensor([length + right_pad]).to(audio_data.device)
        # print(audio_data.shape)
        z = self.encoder(audio_data)
        vae_results = self.quantizer(z, n_quantizers, **kwargs)
        indices_lens = torch.ceil(audio_lengths / self.encoder_frame).long()
        return vae_results, indices_lens
    def encode_to_dist(
        self,
        audio_data: torch.Tensor,
        audio_lengths: torch.Tensor = None,
        ...
        **kwargs,
        ...
        z = self.quantizer.pre_module(z)  # B, T, D
        dist = self.quantizer.proj_in(z)
        return dist

    def decode(self, z: torch.Tensor):
        z = self.quantizer.decode(z)
        return self.decoder(z)
    def decode_indices(self, indices: torch.Tensor, feature_lengths):
        if indices.ndim == 2:
            indices = indices[None]

        z = self.quantizer.decode(indices)
        audio_lengths = feature_lengths * self.decoder_frame
    ...
    def forward(
        ...
        **kwargs,
        ...
        vq_results, indices_lens = self.encode(audio_data, n_quantizers, **kwargs)
        # import pdb; pdb.set_trace()
        audio_len = indices_lens * self.decoder_frame
        x = self.decode(vq_results.latent)
        return x[..., :audio_len], vq_results